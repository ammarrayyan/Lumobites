...
export default function AffiliatesManagement({ adminKey, onUnauthorized }: AffiliatesManagementProps) {
  ...
  if (loading) return <div className="text-white/50 text-center py-12">Loading affiliates data...</div>;
    ...
    <div className="space-y-8 animate-fade-in text-white">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#222] p-6 rounded-2xl border border-white/5 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-5">
            <Activity size={48} className="text-[#64b3f4]" />
          </div>
          <p className="text-xs text-white/50 mb-1">Affiliate Link Clicks</p>
          <p className="text-3xl font-bold text-white">{totalClicks}</p>
          <p className="text-[10px] text-white/40 mt-1">{pendingCount} pending applications</p>
        </div>

        <div className="bg-[#222] p-6 rounded-2xl border border-white/5 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-5">
            <Users size={48} className="text-[#c2e59c]" />
          </div>
          <p className="text-xs text-white/50 mb-1">Active PRO Referrals</p>
          <p className="text-3xl font-bold text-[#c2e59c]">{totalSubscribers}</p>
          <p className="text-[10px] text-white/40 mt-1">{approvedCount} approved partners</p>
        </div>

        <div className="bg-[#222] p-6 rounded-2xl border border-white/5 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-5">
            <DollarSign size={48} className="text-[#64b3f4]" />
          </div>
          <p className="text-xs text-white/50 mb-1">Total Payouts Sent</p>
          <p className="text-3xl font-bold text-[#64b3f4]">${totalPaidOut.toFixed(2)}</p>
          <p className="text-[10px] text-white/40 mt-1">Paid out via PayPal</p>
        </div>
        ...
        <div className="bg-[#222] p-6 rounded-2xl border border-white/5 relative overflow-hidden">
          ...
          <p className="text-xs text-white/50 mb-1">Remaining Balance Owed</p>
          <p className="text-3xl font-bold text-amber-400">${totalOwed.toFixed(2)}</p>
          <p className="text-[10px] text-white/40 mt-1">Owed to all partners</p>
        </div>
      </div>
      ...
      <div className="bg-[#222] rounded-2xl border border-white/5 overflow-hidden">
        <div className="p-6 border-b border-white/5 flex justify-between items-center bg-black/10">
          <div>
            <h3 className="text-lg font-bold text-white">Affiliate Partner Network</h3>
            <p className="text-xs text-white/40 mt-0.5">Manage partner applications, view recurring clicks/subscribers, and issue PayPal payouts.</p>
          </div>
          {pendingCount > 0 && (
            <span className="bg-amber-500/20 border border-amber-500/30 text-amber-400 text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-full animate-pulse">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-white/70">
            <thead className="bg-black/20 text-white/50 font-bold uppercase text-[10px] tracking-wider border-b border-white/5">
              <tr>
            ...
            <tbody className="divide-y divide-white/5">
              {affiliates.map((affiliate: any) => (
                <tr key={affiliate.id} className="hover:bg-white/5 transition-colors">
                  ...
                  {/* Name & PayPal details */}
                  <td className="p-4">
                    <div className="font-bold text-white mb-0.5">{affiliate.full_name}</div>
                    <div className="text-xs text-white/40 font-medium mb-1.5">{affiliate.email}</div>
                    
                    {affiliate.paypal_email && (
                      <div className="text-[10px] bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded inline-flex items-center gap-1 font-bold">
                  ...
                  <td className="p-4 max-w-[280px]">
                    {affiliate.promotion_method && (
                      <div className="text-xs text-white mb-1">
                        <strong className="text-white/40">Promo:</strong> {affiliate.promotion_method}
                      </div>
                    )}
                    {affiliate.bio ? (
                      <div className="text-[11px] text-white/50 italic leading-relaxed line-clamp-2">
                        &ldquo;{affiliate.bio}&rdquo;
                      </div>
                    ) : (
                      <span className="text-[10px] text-white/20">No bio provided</span>
                    )}
                  </td>
                  ...
                  {/* Active Referrals */}
                  <td className="p-4 text-center font-bold text-[#c2e59c]">
                  ...
                  <td className="p-4 font-bold">
                    {affiliate.status === 'approved' ? (
                      <span className={(affiliate.stats?.unpaidBalance || 0) > 0 ? 'text-amber-400' : 'text-white/40'}>
                  ...
                  <td className="p-4">
                    {affiliate.status === 'pending' && (
                      <span className="bg-amber-500/10 border border-amber-500/30 text-amber-400 text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-full">
                        Pending
                      </span>
                    )}
                    {affiliate.status === 'approved' && (
                      <div className="flex flex-col items-start gap-1">
                        <span className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-full">
                          Approved
                        </span>
                        <div className="text-[9px] text-[#64b3f4] font-bold flex items-center gap-1 mt-0.5">
                          code: {affiliate.referral_code}
                          <button onClick={() => copyToClipboard(affiliate.referral_code)} className="text-white/40 hover:text-white transition-colors" title="Copy Promo Link">
                            <Copy size={10} />
                          </button>
                        </div>
                      </div>
                    ...
                    {affiliate.status === 'rejected' && (
                      <span className="bg-red-500/10 border border-red-500/30 text-red-400 text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-full">
                        Rejected
                      </span>
                    )}
                  </td>
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      {affiliate.status === 'pending' && (
                        <>
                          ...
                          <button
                            ...
                            className="bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-400 text-xs font-bold px-3 py-1.5 rounded-lg transition-all cursor-pointer"
                          >
                            Reject
                          </button>
                        </>
                      ...
                      <button
                        ...
                        className="text-white/30 hover:text-red-400 transition-colors p-1.5 rounded-lg hover:bg-white/5 cursor-pointer"
      ...
      {rejectModalAffiliate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-[#1a1a1a] p-6 rounded-2xl border border-white/10 max-w-md w-full text-white shadow-2xl flex flex-col gap-5">
            <div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={isRejecting}
                className="flex-1 bg-red-600 hover:bg-red-700 text-white font-bold py-2.5 rounded-xl text-xs transition-colors disabled:opacity-50"
              >
      ...
      {payModalAffiliate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-[#1a1a1a] p-6 rounded-2xl border border-white/10 max-w-sm w-full text-white shadow-2xl flex flex-col gap-5">
            <div>
            ...
            <div className="bg-black/40 border border-white/5 rounded-xl p-4 space-y-1.5 text-xs text-white/60">
              <p>👤 <strong className="text-white">Partner:</strong> {payModalAffiliate.full_name}</p>
              <p>💳 <strong className="text-white">PayPal Address:</strong> {payModalAffiliate.paypal_email || 'Not provided'}</p>
              <p>⏳ <strong className="text-white">Current Balance Owed:</strong> <span className="text-amber-400 font-bold">${payModalAffiliate.stats?.unpaidBalance.toFixed(2)}</span></p>
            </div>
      ...
      {deleteModalAffiliate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-[#1a1a1a] p-6 rounded-2xl border border-red-500/20 max-w-sm w-full text-white shadow-2xl flex flex-col gap-5">
            ...
            <p className="text-xs text-white/60 leading-relaxed">
              Are you sure you want to permanently delete <strong className="text-white">{deleteModalAffiliate.full_name}</strong>'s affiliate account? 
              <br/><br/>
