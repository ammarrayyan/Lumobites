...
export default function AccountManagement({ adminKey, onUnauthorized }: { adminKey: string, onUnauthorized: () => void }) {
  ...
  if (loading) {
    return <div className="text-white/60 animate-pulse text-center py-12">Loading accounts...</div>;
  }
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 border-b border-white/10 pb-4">
        <h2 className="text-xl font-semibold text-white">All Accounts ({users.length})</h2>
        <input
          type="text"
          placeholder="Search by email..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full md:w-64 bg-black/50 border border-white/10 rounded-lg p-2 text-white focus:outline-none focus:border-[#64b3f4] transition-colors text-sm"
        />
      </div>
        ...
        <table className="w-full text-left text-sm text-white/80">
          ...
          <tbody>
            {filteredUsers.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-4 py-8 text-center text-white/50">No accounts found matching "{search}"</td>
              </tr>
            ) : (
              filteredUsers.map(user => (
                <tr key={user.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    {user.is_pro ? (
                      <span className="px-2 py-1 rounded bg-purple-500/20 text-purple-400 text-xs font-bold">PRO</span>
                    ) : (
                      <span className="px-2 py-1 rounded bg-white/10 text-white/60 text-xs font-medium">Free</span>
                    )}
                  </td>
