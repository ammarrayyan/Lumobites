...
export function PetSittingContent() {
  ...
  const fetchVetClinics = async () => {
    try {
      ...
      console.error('Failed to fetch vet clinics');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F7F3EE] font-sans selection:bg-[#8B5E3C] selection:text-white flex flex-col relative">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
          ...
          {activeTab === 'find' && (
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs sm:text-sm font-bold bg-[#C17D3C] hover:bg-[#B06D2B] active:bg-[#9B5C20] text-white shadow-[0_4px_14px_rgba(193,125,60,0.35)] transition-all duration-200 cursor-pointer shrink-0 active:scale-95 border border-[#B06D2B]/30"
            >
                ...
                <div className="flex-1 flex flex-col gap-1">
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      className={`bg-[#FAF6F4] hover:bg-[#E8DDD4] border border-[#E8DDD4] rounded-xl px-3 py-2 md:px-4 md:py-3 text-[16px] md:text-sm text-[#8B5E3C] font-semibold flex items-center gap-2 transition duration-200 shrink-0 shadow-sm ${
                  ...
                  {/* Location Verification Status (Positioned Directly Under Location Input) */}
                  {searchZip.trim() && (
                    <div className="mt-1 px-1 min-h-[20px]">
                      {isGeocoding ? (
                        <span className="text-[#8B5E3C] text-xs font-semibold flex items-center gap-2">
                          <svg className="animate-spin h-3.5 w-3.5 text-[#8B5E3C]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                          Locating...
                        </span>
                      ) : searchLocationError ? (
                        <span className="text-red-600 text-xs font-semibold flex items-center gap-1.5">
                          <XCircle className="w-3.5 h-3.5 text-red-600" /> {searchLocationError}
                        </span>
                      ) : searchLocationName ? (
                        <span className="text-green-700 text-xs font-semibold flex items-center gap-1.5">
                          <CheckCircle className="w-3.5 h-3.5 text-green-700" /> {searchLocationName}
                        </span>
                      ) : null}
                    </div>
                  )}
                  ...
                  <div className="relative">
                    <input 
                            ...
                            {sitterLocationOptions.map((opt, i) => (
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setSitterCity(formatPublicCity(opt.city || opt.formatted_address));
                                  setSitterLocationVerified(true);
                                  setSitterLocationInput(formatPublicCity(opt.city || opt.formatted_address));
                                }}
                      ...
                      <p className="text-[11px] text-gray-500 mt-1 flex items-center gap-1">
                        <Lock className="w-3 h-3 text-[#8B5E3C] inline shrink-0" />
                        <span><strong>Privacy Guarantee:</strong> Only your city and state (e.g. <em>Columbus, OH</em>) will be shown publicly. Your exact street address is kept strictly private for distance matching only.</span>
                      </p>
                </div>
                <div>
      ...
      {inquiringClinic && (
        <VetClinicInquiryModal
          clinic={inquiringClinic}
          ownerEmail={reqEmail}
          onClose={() => setInquiringClinic(null)}
        />
      )}
...
// ─── Vet Clinic Inquiry Modal (additive, self-contained) ─────────────────────
interface VetClinicInquiryModalProps {
  clinic: any;
  ownerEmail: string;
  onClose: () => void;
}

function VetClinicInquiryModal({ clinic, ownerEmail, onClose }: VetClinicInquiryModalProps) {
