...
export async function GET(request: NextRequest) {
  try {
    ...
    const { reviews, avgRating, reviewCount } = await getPartnerReviews(clinic_id, 'vet');

    const { data: clinic } = await supabaseAdmin
      .from('vet_clinics')
      .select('id, clinic_name, city, state, org_photo_url, avg_rating, review_count')
      .eq('id', clinic_id)
      .maybeSingle();

    return NextResponse.json(
      { reviews, clinic, avg_rating: avgRating || clinic?.avg_rating || 0, review_count: reviewCount || clinic?.review_count || 0 },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { clinic_id, owner_email, owner_name, rating, review_text } = body;

    if (!clinic_id || !owner_email || !rating || !review_text) {
